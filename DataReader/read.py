...
from DataReader.util import loadInfo, zRotMatrix, proj, mesh2UV, uv_to_pixel
from DataReader.IO import readOBJ, readPC2Frame, writeOBJ
...
class DataReader:
	...
	def __init__(self, config):
		# Data paths
		self.SRC = os.path.abspath(config['data_val']) 
		#self.SRC = os.path.abspath('/home/cxh/mnt/nas/Documents/dataset/CLOTH3D/val_t1/') 
		# SMPL model
		...
		#}
		self.smpl = {
			'f': SMPLModel(config['smpl_f']),
			'm': SMPLModel(config['smpl_m'])
		}
...
if __name__ == '__main__':
	sample = '00001'
	frame = 0
	abs_path = '/home/cxh/mnt/nas/Documents/dataset/CLOTH3D/train/'
	reader = DataReader()
	info = reader.read_info(sample)	
	trans = info['trans']	
	print(f' trans : {trans.shape}')
	#print(f' info : {info}')
	num_frame = trans.shape[-1]
	# Read human vertices and faces
	h_v, h_f = reader.read_human(sample, frame)	
	print(f' h_f : {h_f.shape}')	
	print(f' h_v : {h_v.shape}')	
	garments = list(info['outfit'].keys())
	for i,garment in enumerate(garments):
		for j in range(num_frame):
			g_v = reader.read_garment_vertices(sample,garment,frame)
			g_f = reader.read_garment_topology(sample,garment)
			g_f = quads2tris(g_f)
			g_vt, g_ft = reader.read_garment_UVMap(sample, garment)

			#g_c = reader.read_garment_vertex_colors(sample, garment, F, Vt, Ft)
		
			#print(f' g_v : {g_v.shape}')
